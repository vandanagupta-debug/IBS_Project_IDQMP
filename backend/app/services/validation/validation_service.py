...
def build_validation_summary(dataset: Dataset) -> ValidationSummaryOut:
    ...
    record(
        "Duplicate Detection", "(all columns)", dup_count > 0, dup_count, "Duplicates"
    )
    ...
    for col in df.columns:
        ...
        if 0 < looks_numeric.mean() < 1:
            ...
            record(
                "Schema / Type Consistency", str(col), True, affected, "Schema Errors"
            )
    ...
    for col in date_cols:
        ...
        record(
            "Freshness Validation", str(col), future > 0, future, "Freshness Violations"
        )
    ...
    breakdown_out = [
        BreakdownItemOut(name=k, value=v)
